...
public class ServicioHabitoIAImpTest {
    ...
    private static final String RESPUESTA_JSON = "{\"nombre\":\"Comer saludable\",\"descripcion\":\"Preparar comidas saludables diariamente\",\"frecuencia\":\"Diario\",\"categoria\":\"Salud\"}";
    ...
    @Test
    public void recomendarDeberiaRetornarHabitoSugerido()
    ...
    @Test
    public void recomendarDeberiaLimpiarRespuestaJsonConMarkdown()
            throws JsonProcessingException {
    ...
    public void recomendarDeberiaLanzarErrorCuandoLaRespuestaNoEsJson()
        ...
        try {
            ...
            assertThat(e.getMessage(), equalTo( "No se pudo convertir la respues correctamente Respuesta inválida"));
        }
    }

    private void dadoQueElServicioIAResponde(String respuesta)
            throws JsonProcessingException {
        when(servicioIAMock.preguntar(anyString())).thenReturn(respuesta);

    }

    private HabitoSugeridoDTO cuandoRecomiendoUnHabito()
            throws JsonProcessingException {
        return servicioHabitoIA.recomendar(OBJETIVO);
    }

    private void entoncesElHabitoTieneLosDatosEsperados(
            HabitoSugeridoDTO respuesta) {
